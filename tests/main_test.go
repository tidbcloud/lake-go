...
func init() {
	s := os.Getenv("TEST_LAKE_DSN")
	if s != "" {
		dsn = s
	}

	serverVersion = getVersion("LAKE_VERSION")
	driverVersion = getVersion("LAKE_GO_VERSION")
	...
	//dsn = "http://lake:lake@localhost:8000?presign=on"
}
...
func TestLakeSuite(t *testing.T) {
	suite.Run(t, new(LakeTestSuite))
}

type LakeTestSuite struct {
	suite.Suite
	cfg    *dc.Config
	table  string
	table2 string
...
func (s *LakeTestSuite) SetupSuite() {
	var err error
...
func (s *LakeTestSuite) Conn() *sql.Conn {
	db := sql.OpenDB(s.cfg)
...
func (s *LakeTestSuite) SetupTest() {
	t := s.T()
	s.r = require.New(t)
	db := sql.OpenDB(s.cfg)
	defer db.Close()
...
func (s *LakeTestSuite) TearDownTest() {
	// t := s.T()
	s.SetupSuite()
	db := sql.OpenDB(s.cfg)
	defer db.Close()
...
func (s *LakeTestSuite) TestVersion() {
	s.SetupSuite()
	db := sql.OpenDB(s.cfg)
	defer db.Close()
	...
	s.r.NoError(err)
	s.T().Logf("connected to lake: %s\n", result)
}

// For load balance test
func (s *LakeTestSuite) TestCycleExec() {
	db := sql.OpenDB(s.cfg)
	defer db.Close()
...
func (s *LakeTestSuite) TestQuoteStringQuery() {
	db := sql.OpenDB(s.cfg)
	defer db.Close()
...
func (s *LakeTestSuite) TestBasicSelect() {
	db := sql.OpenDB(s.cfg)
	defer db.Close()
	query := "SELECT ?"
...
func (s *LakeTestSuite) TestSelectMultiPage() {
...
func (s *LakeTestSuite) TestPrepare() {
...
func (s *LakeTestSuite) TestBatchInsertOld() {
	if semver.Compare(driverVersion, "v0.9.0") > 0 {
		return
	}
...
func (s *LakeTestSuite) TestServerError() {
	db := sql.OpenDB(s.cfg)
	defer db.Close()
...
func (s *LakeTestSuite) TestTransactionCommit() {
	db := sql.OpenDB(s.cfg)
	defer db.Close()
	tx, err := db.Begin()
	s.r.NoError(err)
...
func (s *LakeTestSuite) TestTransactionRollback() {
	db := sql.OpenDB(s.cfg)
	defer db.Close()
	tx, err := db.Begin()
	s.r.NoError(err)
