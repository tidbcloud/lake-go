...
import (
	...
	golake "github.com/tidbcloud/lake-go"
)
...
func NewResponse(rows *sql.Rows, err error) (res *Response) {
	...
	for rows.Next() {
		...
		res.Values = append(res.Values, golake.LastRawRow(rows))
	}
...
func main() {
	dataSource := os.Getenv("LAKE_DSN")
	if dataSource == "" {
		slog.Error("LAKE_DSN is not set")
		return
	}
	...
	cfg, err := golake.ParseDSN(dataSource)
	if err != nil {
